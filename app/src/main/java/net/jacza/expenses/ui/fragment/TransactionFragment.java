            ...
            public void onSwiped(@NonNull RecyclerView.ViewHolder viewHolder, int direction) {
                ...
                if (transactions.size() > 0) {
                    if (position == 0) {
                        ...
                        if (position == transactions.size()) adapter.notifyItemChanged(transactions.size() - 1); // Update the new last if the previous last was deleted
                    }
                }
                ...
                Snackbar.make(recyclerView, "Transaction deleted", Snackbar.LENGTH_LONG)
                        .setAction("Undo", v -> {
                            ...
                            if (position == 0) {
                                ...
                                if (position == transactions.size() - 2) adapter.notifyItemChanged(transactions.size() - 1); // Update the new last if inserted before the last
                            }
                        })
